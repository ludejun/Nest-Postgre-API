...
export class IPService {
  ...
  private queryIPByAliyun(ip: IP): Promise<IPLocation> {
    return this.httpService.axiosRef
      ...
      .catch((error) => {
        ...
        console.warn('Aliyun 查询 IP 信息失败！', error);
  ...
  private queryIPByJUHE(ip: IP): Promise<any> {
    return this.httpService.axiosRef
      .get(
        `http://apis.juhe.cn/ip/ipNew?ip=${ip}&key=${APP_CONFIG.COMMON_SERVICE.juheIPAuth}`,
      )
      .then((response) => {
      ...
      .catch((error) => {
        // const message = getMessageFromAxiosError(error);
        console.warn('juhe.cn 查询 IP 信息失败！', error);
        return Promise.reject(error);
      });
  }
